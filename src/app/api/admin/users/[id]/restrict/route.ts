...
function userIdFromPath(pathname: string): string | null {
  const m = pathname.match(/\/api\/admin\/users\/([^/]+)\/restrict/)
  return m?.[1] ?? null
}
...
export const POST = withAdminApi(async ({ staff, req }) => {
  ...
  try {
    await prisma.user.update({
      where: { id },
      data: { accountRestricted: true },
    })
    await addUserTrustEvent(id, "ADMIN_ACCOUNT_RESTRICTED", {
      reason: "Ограничение с админки",
      metadata: { staffId: staff.id },
    })
    await recalculateUserTrust(id)
    await writeAudit({
      actorId: staff.id,
      action: AuditAction.ADMIN_USER_RESTRICTED,
      targetType: "User",
      targetId: id,
      metadata: {},
      ip: extractIp(req),
