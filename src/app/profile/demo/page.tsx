...
import { AutoResponderSettings } from "@/components/profile/AutoResponderSettings"
...
const defaultProfile: Profile = {
  ...
  about: "Графический дизайн, логотипы и аккуратные объявления на Отиве.",
}
...
const SELLER_CABINET_PLAN = {
  id: "seller-pro", label: "Кабинет продавца Pro", price: 499, days: 30,
  desc: "Мониторинг объявлений, динамика спроса и подсказки по продвижению",
}

type Deal = { id: string; title: string; price: number; date: string; counterpart: string; status: "completed" | "cancelled"; category: string }

const DEMO_DEALS_SOLD: Deal[] = [
  { id: "ds1", title: "Логотип для кофейни", price: 8500, date: "18 апр 2025", counterpart: "Кирилл Н.", status: "completed", category: "Услуги" },
  { id: "ds2", title: "Фирменный стиль стартапа", price: 24000, date: "22 мар 2025", counterpart: "Юлия С.", status: "completed", category: "Услуги" },
  { id: "ds3", title: "Брендинг ресторана", price: 15000, date: "5 мар 2025", counterpart: "Артём В.", status: "completed", category: "Услуги" },
  { id: "ds4", title: "Логотип для бизнеса", price: 6000, date: "14 фев 2025", counterpart: "Мария Ф.", status: "completed", category: "Услуги" },
]

const DEMO_DEALS_BOUGHT: Deal[] = [
  { id: "db1", title: "MacBook Pro 14\" M3", price: 185000, date: "12 апр 2025", counterpart: "Алексей М.", status: "completed", category: "Электроника" },
  { id: "db2", title: "Стол для переговоров", price: 28000, date: "2 мар 2025", counterpart: "Ольга С.", status: "completed", category: "Дом и интерьер" },
  { id: "db3", title: "Курс по типографике", price: 3200, date: "10 янв 2025", counterpart: "Онлайн-школа", status: "cancelled", category: "Услуги" },
]

const DEMO_REVIEWS: Review[] = [
  { id: "dr1", author: "Кирилл Н.", avatar: "К", rating: 5, date: "10 апр 2025", text: "Отличный продавец! Всё чётко по описанию, быстро вышел на связь. Рекомендую.", listingTitle: "Услуги дизайна", role: "buyer", helpful: 7 },
  { id: "dr2", author: "Юлия С.", avatar: "Ю", rating: 5, date: "22 мар 2025", text: "Очень приятная сделка. Продавец честный, без скрытых нюансов. Буду обращаться ещё.", listingTitle: "Логотип для бизнеса", role: "buyer", helpful: 4 },
  { id: "dr3", author: "Артём В.", avatar: "А", rating: 4, date: "5 мар 2025", text: "Хорошо, но немного долго ждал ответа. В итоге всё решили, товар соответствует.", listingTitle: "Услуги дизайна", role: "buyer", helpful: 2 },
  { id: "dr4", author: "Мария Ф.", avatar: "М", rating: 5, date: "14 фев 2025", text: "Всё идеально. Профессиональный подход, красивый результат. Спасибо!", listingTitle: "Фирменный стиль", role: "buyer", helpful: 11 },
]

function loadBalance(): number {
  if (typeof window === "undefined") return DEMO_START_BALANCE
  const raw = localStorage.getItem("otiva-balance")
  if (raw === null) { localStorage.setItem("otiva-balance", String(DEMO_START_BALANCE)); return DEMO_START_BALANCE }
  return Number(raw)
}
...
export default function DemoProfilePage() {
  ...
  const [dealTab, setDealTab] = useState<"sold" | "bought">("sold")
  ...
  function saveBalance(v: number) { setBalance(v); localStorage.setItem("otiva-balance", String(v)) }
  function savePromos(p: ActivePromo[]) { setPromos(p); localStorage.setItem("otiva-promos", JSON.stringify(p)) }
  ...
  function updateField(field: keyof Profile, value: string) { setProfile((c) => ({ ...c, [field]: value })); setSaved(false) }
  function saveProfile() { localStorage.setItem("otiva-demo-user", JSON.stringify(profile)); setIsEditing(false); setSaved(true) }

  function topup(amount: number) { saveBalance(balance + amount); setTopupOpen(false); showToast("Пополнено на " + amount + " ₽") }

  function buyPromo(listingId: string, plan: typeof PROMO_PLANS[0]) {
    if (balance < plan.price) { setPromoOpen(null); setTopupOpen(true); showToast("Недостаточно средств — пополните баланс"); return }
  ...
  function cancelPromo(listingId: string) { savePromos(promos.filter((p) => p.listingId !== listingId)); showToast("Продвижение отключено") }

  function buySellerCabinet() {
    if (balance < SELLER_CABINET_PLAN.price) { setTopupOpen(true); showToast("Недостаточно средств — пополните баланс"); return }
  ...
  function cancelSellerCabinet() { saveSellerCabinet(null); showToast("Кабинет продавца отключен") }
  ...
  const totalViews = myListings.reduce((s, _, i) => s + (listingViews[i] || 0), 0)
  const totalFavorites = myListings.reduce((s, _, i) => s + (listingFavorites[i] || 0), 0)
  const totalChats = myListings.reduce((s, _, i) => s + (listingChats[i] || 0), 0)
  const conversion = totalViews ? Math.round((totalChats / totalViews) * 1000) / 10 : 0
  const sellerCabinetActive = Boolean(sellerCabinet)
  const monitoredListings = myListings.map((listing, i) => ({
    listing, views: listingViews[i] || 0, favorites: listingFavorites[i] || 0, chats: listingChats[i] || 0,
    demand: (listingViews[i] || 0) > 2500 ? "Высокий" : (listingViews[i] || 0) > 900 ? "Средний" : "Нужен буст",
    recommendation: (listingChats[i] || 0) < 10 ? "Поднять в топ" : (listingFavorites[i] || 0) > 300 ? "Ответить покупателям" : "Обновить описание",
  }))
  const profileStats = [
    { value: "4.9", label: "рейтинг", hint: `${DEMO_REVIEWS.length} отзыва` },
  ...
  return (
    <>
      ...
      {/* Topup modal */}
      {topupOpen && (
        <div className="fixed inset-0 z-[190] flex items-center justify-center bg-zinc-950/50 px-4" onClick={(e) => { if (e.target === e.currentTarget) setTopupOpen(false) }}>
          <div className="w-full max-w-sm rounded-3xl bg-white shadow-2xl">
            <div className="flex items-center justify-between border-b border-zinc-100 px-6 py-5">
              <div><h3 className="text-xl font-semibold text-zinc-950">Пополнить баланс</h3><p className="mt-0.5 text-sm text-zinc-500">Сейчас: {balance} ₽</p></div>
            ...
            <div className="p-6">
              ...
              <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
                {TOPUP_AMOUNTS.map((amt) => (
                  <button key={amt} onClick={() => topup(amt)} className="rounded-2xl border-2 border-zinc-200 py-3.5 text-sm font-bold text-zinc-950 transition hover:border-[hsl(var(--otiva-orange))] hover:bg-[hsl(var(--otiva-orange)/0.08)] hover:text-[hsl(var(--otiva-orange))]">
                    {amt} ₽
                  </button>
                ))}
              </div>
              <p className="mt-4 rounded-2xl bg-zinc-50 px-4 py-3 text-xs text-zinc-500">Демо-режим: средства зачисляются мгновенно без реальной оплаты.</p>
            </div>
          </div>
        </div>
      )}
      ...
      {/* Promo modal */}
      {promoOpen && (
        <div className="fixed inset-0 z-[190] flex items-center justify-center bg-zinc-950/50 px-4" onClick={(e) => { if (e.target === e.currentTarget) setPromoOpen(null) }}>
          <div className="w-full max-w-sm rounded-3xl bg-white shadow-2xl">
            <div className="flex items-center justify-between border-b border-zinc-100 px-6 py-5">
              <div><h3 className="text-xl font-semibold text-zinc-950">Продвижение</h3><p className="mt-0.5 text-sm text-zinc-500">Баланс: {balance} ₽</p></div>
            ...
            <div className="space-y-2 p-6">
              {PROMO_PLANS.map((plan) => (
                <button key={plan.id} onClick={() => buyPromo(promoOpen, plan)} className="flex w-full flex-col gap-3 rounded-2xl border-2 border-zinc-200 p-4 text-left transition hover:border-[hsl(var(--otiva-orange))] hover:bg-[hsl(var(--otiva-orange)/0.05)] sm:flex-row sm:items-center sm:justify-between">
                  <div><p className="font-semibold text-zinc-950">{plan.label}</p><p className="mt-0.5 text-sm text-zinc-500">{plan.desc}</p></div>
        ...
        <section className="lg:hidden">
          <div className="rounded-[30px] bg-zinc-950 p-5 text-white shadow-2xl shadow-zinc-950/20">
            ...
            <div className="mt-6">
              ...
              <p className="mt-2 text-sm leading-6 text-white/70">На Отиве с 2026 года · частное лицо</p>
              <p className="mt-1 text-sm text-white/70">Номер профиля 431 774 093</p>
            </div>
            <div className="mt-5 grid grid-cols-2 gap-3">
              <div className="rounded-3xl bg-white/10 p-4">
                <p className="text-2xl font-semibold">4,9 ★</p>
                <p className="mt-1 text-sm text-white/55">{DEMO_REVIEWS.length} отзыва</p>
              </div>
              <div className="rounded-3xl bg-white/10 p-4">
          ...
          <div className="mt-4 rounded-[28px] bg-zinc-100 p-4">
            <div className="flex items-center justify-between rounded-3xl bg-white p-4 shadow-sm">
              <div>
                ...
                <p className="mt-1 text-sm text-zinc-500">Отива Кошелёк</p>
              </div>
              <button onClick={() => setTopupOpen(true)} className="rounded-2xl bg-zinc-950 px-5 py-3 text-sm font-semibold text-white">Пополнить</button>
            </div>
            <div className="mt-3 grid grid-cols-2 gap-3">
              ...
              <div className="rounded-3xl bg-white p-4 shadow-sm">
                ...
                <p className="mt-1 text-sm text-zinc-500">И ещё 5 продуктов</p>
              </div>
            </div>
          </div>
          ...
          <div className="mt-4 overflow-hidden rounded-[28px] border border-zinc-200 bg-white">
            {[
              ...
              { title: "Мои отзывы", desc: `${DEMO_REVIEWS.length} отзыва · 4.9 ★`, href: "#reviews-mobile" },
          ...
          <div className="mt-4 rounded-[28px] border border-zinc-200 bg-white p-5">
            ...
            <button type="button" className="mt-4 w-full rounded-2xl bg-zinc-100 px-4 py-3 text-sm font-semibold text-[hsl(var(--otiva-blue))]">Перейти к адресам</button>
          </div>
        </section>
          ...
          <aside className="hidden space-y-4 lg:block lg:sticky lg:top-24 lg:h-fit">
            <div className="min-w-0 rounded-[28px] border border-zinc-200 bg-white p-5 shadow-sm sm:rounded-[32px] sm:p-6">
              ...
              <div className="mt-3 flex items-center gap-2">
                <span className="text-lg font-bold text-zinc-950">4.9</span>
                <div className="flex gap-0.5">{[1,2,3,4,5].map((s) => <span key={s} className="text-[hsl(var(--otiva-orange))]">★</span>)}</div>
                <span className="text-sm text-zinc-500">({DEMO_REVIEWS.length})</span>
              </div>
              <div className="mt-4 rounded-2xl bg-[hsl(var(--otiva-mint)/0.12)] px-4 py-2.5 text-sm font-semibold leading-5 text-[hsl(var(--otiva-mint))]">Профиль подтвержден</div>
            </div>
            ...
            <div className="min-w-0 rounded-[28px] border-2 border-zinc-200 bg-white p-5 shadow-sm sm:rounded-[32px] sm:p-6">
              ...
              <button onClick={() => setTopupOpen(true)} className="mt-4 w-full rounded-2xl bg-[hsl(var(--otiva-orange))] px-4 py-3 text-sm font-semibold text-white shadow-sm transition hover:bg-[hsl(var(--otiva-orange)/0.9)]">
                + Пополнить
              </button>
              {promos.length > 0 && (
          ...
          <section className="flex min-w-0 flex-col gap-6">
            ...
            <div id="profile-form-mobile" className="order-4 min-w-0 rounded-[28px] border border-zinc-200 bg-zinc-50 p-5 shadow-inner sm:rounded-[32px] sm:p-6">
              <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                ...
                <button onClick={() => (isEditing ? saveProfile() : setIsEditing(true))} className="w-full rounded-2xl bg-zinc-950 px-5 py-3 text-sm font-semibold text-white hover:bg-zinc-800 sm:w-auto">
                  {isEditing ? "Сохранить" : "Редактировать"}
                </button>
              </div>
              <div className="mt-6 grid gap-4 md:grid-cols-2">
                {([["name","Имя"],["phone","Телефон"],["email","Email"],["city","Город"]] as const).map(([field, label]) => (
                  <label key={field} className="block">
                    <span className="text-sm font-medium text-zinc-600">{label}</span>
                    <input disabled={!isEditing} value={profile[field]} onChange={(e) => updateField(field, e.target.value)} className="mt-2 w-full rounded-2xl border border-zinc-200 bg-white px-4 py-3 text-sm outline-none disabled:bg-zinc-100" />
                  </label>
                ))}
            ...
            <section id="seller-cabinet-mobile" className="order-2 rounded-[28px] border border-zinc-200 bg-white p-5 shadow-sm sm:rounded-[32px] sm:p-6">
              <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                <div className="min-w-0">
                  ...
                  <p className="mt-1 max-w-2xl text-sm leading-6 text-zinc-500">Следите за объявлениями, спросом, чатами и рекомендациями по продвижению.</p>
                </div>
                <div className="shrink-0 rounded-3xl bg-zinc-50 p-4 lg:w-72">
                  ...
                  {sellerCabinetActive ? (
                    <div className="mt-4">
                      <p className="rounded-2xl bg-white px-3 py-2 text-sm font-medium text-zinc-600">Активен до {sellerCabinet?.until}</p>
                      <button onClick={cancelSellerCabinet} className="mt-2 w-full rounded-2xl border border-zinc-200 bg-white px-4 py-2.5 text-sm font-semibold text-zinc-600 transition hover:border-zinc-300 hover:text-zinc-950">Отключить</button>
                    </div>
                  ) : (
                    <button onClick={buySellerCabinet} className="mt-4 w-full rounded-2xl bg-zinc-950 px-4 py-3 text-sm font-semibold text-white transition hover:bg-zinc-800">Подключить на 30 дней</button>
                  )}
                </div>
              </div>
              ...
              <div className="mt-5 overflow-hidden rounded-3xl border border-zinc-100">
                <div className="hidden grid-cols-[minmax(0,1.7fr)_0.7fr_0.7fr_0.7fr_1fr] gap-3 bg-zinc-50 px-4 py-3 text-xs font-semibold uppercase tracking-wide text-zinc-400 lg:grid">
                  <span>Объявление</span><span>Просмотры</span><span>Избранное</span><span>Чаты</span><span>Рекомендация</span>
                </div>
                <div className="divide-y divide-zinc-100">
                  {monitoredListings.map((item) => (
                    <div key={item.listing.id} className="grid gap-3 px-4 py-4 lg:grid-cols-[minmax(0,1.7fr)_0.7fr_0.7fr_0.7fr_1fr] lg:items-center">
                      ...
                      <div><p className="text-xs text-zinc-400 lg:hidden">Просмотры</p><p className="font-semibold text-zinc-950">{sellerCabinetActive ? item.views.toLocaleString("ru-RU") : "—"}</p></div>
                      <div><p className="text-xs text-zinc-400 lg:hidden">Избранное</p><p className="font-semibold text-zinc-950">{sellerCabinetActive ? item.favorites.toLocaleString("ru-RU") : "—"}</p></div>
                      <div><p className="text-xs text-zinc-400 lg:hidden">Чаты</p><p className="font-semibold text-zinc-950">{sellerCabinetActive ? item.chats : "—"}</p></div>
                      <div className="min-w-0">
            ...
            {/* Deal History */}
            <section className="order-1 rounded-[28px] border border-zinc-200 bg-white p-5 shadow-sm sm:rounded-[32px] sm:p-6">
              <div className="flex items-center justify-between gap-4">
                <div>
                  <h2 className="text-xl font-semibold text-zinc-950 sm:text-2xl">История сделок</h2>
                  <p className="mt-1 text-sm text-zinc-500">Завершённые покупки и продажи.</p>
                </div>
              </div>
              {/* Tabs */}
              <div className="mt-4 flex gap-2">
                <button
                  onClick={() => setDealTab("sold")}
                  className={`rounded-xl px-4 py-2 text-sm font-semibold transition ${dealTab === "sold" ? "bg-zinc-950 text-white" : "bg-zinc-100 text-zinc-600 hover:bg-zinc-200"}`}
                >
                  Продал ({DEMO_DEALS_SOLD.length})
                </button>
                <button
                  onClick={() => setDealTab("bought")}
                  className={`rounded-xl px-4 py-2 text-sm font-semibold transition ${dealTab === "bought" ? "bg-zinc-950 text-white" : "bg-zinc-100 text-zinc-600 hover:bg-zinc-200"}`}
                >
                  Купил ({DEMO_DEALS_BOUGHT.length})
                </button>
              </div>
              {/* Deals list */}
              <div className="mt-4 divide-y divide-zinc-100 overflow-hidden rounded-2xl border border-zinc-100">
                {(dealTab === "sold" ? DEMO_DEALS_SOLD : DEMO_DEALS_BOUGHT).map((deal) => (
                  <div key={deal.id} className="flex items-center gap-4 px-4 py-3.5">
                    <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl bg-zinc-100 text-lg">
                      {dealTab === "sold" ? "↑" : "↓"}
                    </div>
                    <div className="min-w-0 flex-1">
                      <p className="truncate font-semibold text-zinc-950">{deal.title}</p>
                      <p className="mt-0.5 text-xs text-zinc-400">{deal.category} · {deal.counterpart} · {deal.date}</p>
                    </div>
                    <div className="shrink-0 text-right">
                      <p className={`font-semibold ${dealTab === "sold" ? "text-[hsl(var(--otiva-mint))]" : "text-zinc-950"}`}>
                        {dealTab === "sold" ? "+" : "−"}{new Intl.NumberFormat("ru-RU").format(deal.price)} ₽
                      </p>
                      <span className={`text-xs font-medium ${deal.status === "completed" ? "text-zinc-400" : "text-[hsl(var(--otiva-orange))]"}`}>
              ...
              <div className="mt-4 flex flex-wrap gap-3">
                <div className="flex-1 rounded-2xl bg-zinc-50 p-4 text-center">
                  <p className="text-lg font-bold text-zinc-950">
                    +{new Intl.NumberFormat("ru-RU").format(DEMO_DEALS_SOLD.reduce((s, d) => s + (d.status === "completed" ? d.price : 0), 0))} ₽
                  </p>
                  <p className="mt-0.5 text-xs text-zinc-500">выручка</p>
                </div>
                <div className="flex-1 rounded-2xl bg-zinc-50 p-4 text-center">
                  <p className="text-lg font-bold text-zinc-950">
                    {new Intl.NumberFormat("ru-RU").format(DEMO_DEALS_BOUGHT.reduce((s, d) => s + (d.status === "completed" ? d.price : 0), 0))} ₽
                  </p>
                  <p className="mt-0.5 text-xs text-zinc-500">потрачено</p>
                </div>
                <div className="flex-1 rounded-2xl bg-zinc-50 p-4 text-center">
                  <p className="text-lg font-bold text-zinc-950">{DEMO_DEALS_SOLD.length + DEMO_DEALS_BOUGHT.length}</p>
                  <p className="mt-0.5 text-xs text-zinc-500">сделок всего</p>
                </div>
              </div>
            </section>

            {/* Reviews */}
            <section id="reviews-mobile" className="order-1 rounded-[28px] border border-zinc-200 bg-white p-5 shadow-sm sm:rounded-[32px] sm:p-6">
              <div className="flex items-center justify-between gap-4">
                <div>
                  <h2 className="text-xl font-semibold text-zinc-950 sm:text-2xl">Отзывы обо мне</h2>
                  <p className="mt-1 text-sm text-zinc-500">Оценки покупателей по завершённым сделкам.</p>
                </div>
                <span className="shrink-0 rounded-full bg-zinc-100 px-3 py-1 text-sm font-semibold text-zinc-600">{DEMO_REVIEWS.length}</span>
              </div>
