...
function calcRating(reviews: Review[]) {
  ...
  let sum = 0
  for (const r of reviews) { sum += r.rating; dist[r.rating - 1]++ }
...
export function RatingSummary({ reviews }: { reviews: Review[] }) {
  ...
  return (
    <div className="flex flex-col gap-6 sm:flex-row sm:items-center">
      <div className="flex shrink-0 flex-col items-center justify-center rounded-[24px] bg-zinc-50 px-8 py-6 text-center">
          ...
          return (
            <div key={star} className="flex items-center gap-3">
              ...
              <div className="flex-1 h-2 overflow-hidden rounded-full bg-zinc-100">
                <div className="h-2 rounded-full bg-[hsl(var(--nashlo-orange))] transition-all" style={{ width: pct + "%" }} />
              </div>
