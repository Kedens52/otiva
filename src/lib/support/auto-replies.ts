...
export const SUPPORT_AUTO_REPLIES: SupportAutoReply[] = [
  {
    ...
    title:    "Нашло участвует в сделке?",
    keywords: [
      "сделка", "гарантия", "продавец", "покупатель", "оплата", "перевод", "обман",
      "кто отвечает", "участвует", "посредник", "гарант",
    ],
    answer:
      "Нашло — это площадка объявлений. Мы помогаем пользователям размещать объявления, находить предложения и связываться друг с другом. Нашло не является продавцом, покупателем, исполнителем, заказчиком или стороной сделки. Пользователи самостоятельно договариваются об условиях, оплате и передаче товара или оказании услуги.",
  ...
  },
  {
    ...
    answer:
      "На старте Нашло можно разместить первые объявления бесплатно в рамках лимита. Бесплатный лимит нужен, чтобы пользователи могли попробовать сервис. Если лимит закончился, можно выбрать тариф или подключить продвижение.",
    links: [
  ...
  },
  {
    ...
    answer:
      "Платные услуги Нашло относятся только к техническим возможностям платформы: размещению, продвижению, выделению или дополнительному показу объявления. Оплата на Нашло не является оплатой товара, услуги, автомобиля или недвижимости из объявления. Продвижение не гарантирует продажу или сделку.",
  ...
  },
  {
    ...
    answer:
      "Сохраните переписку, ссылку на объявление, данные пользователя и детали оплаты. Отправьте жалобу в поддержку. Нашло не является стороной сделки, но может проверить аккаунт, ограничить нарушителя и принять меры внутри сервиса. При признаках мошенничества также обратитесь в правоохранительные органы.",
    links: [{ label: "Пользовательское соглашение", href: "/legal/user-agreement" }],
    escalationRecommended: true,
  },
  {
...
const TEASER_BY_CATEGORY: Record<string, string> = {
  marketplace: "Похоже, вопрос о роли Нашло в сделках и сервисе.",
  ...
  safety:      "Похоже, вопрос о мошенничестве или о личной встрече с продавцом.",
